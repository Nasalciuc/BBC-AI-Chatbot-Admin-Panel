...
import { claimConversation, getQueue, ApiError } from '@/lib/api'
 ...
 *
 * Rows come from /api/conversations/queue, the same server-side scope the
 * heartbeat counts, so the list and the badge can never disagree — and the
 * visitor who asked for a human out loud is actually in it. No timer is shown
 * on the rows: a countdown would turn the work into a reflex game.
 *
 * On 409 the row belongs to somebody else: a neutral toast names the winner,
 * the row leaves at the next refresh, nothing typed is touched and nothing
 * opens by itself — the human decides what to take next.
 *
 * `canClaim` is false for oversight roles (admin / supervisor / QA): they can
 * see the line, but the claim endpoint would answer 403, and a button that
 * looks alive and is not generates a support ticket.
 */
export function QueueSection({
  ...
  const { data: rows = [] } = useQuery({
    ...
    queryFn: async () => (await getQueue()).data ?? [],
  })

  if (queueCount === 0 && rows.length === 0) return null
